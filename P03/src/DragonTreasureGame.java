...
public class DragonTreasureGame {
	...
	/**
	 * Reads ands parses the information to generate and create the Rooms of the
	 * cave.
	 * 
		...
		try {
			...
			while (fileReader.hasNext()) {
				...
				if (parts.length == 3) { // has a char at end, special type of room
					...
					/*
					 * Switch statements are like if and else ifs but only work on a simple
					 * condition like someChar == 'A'
					 */
					switch (parts[2]) {
					case "S":
	...
	/**
	 * Searches the roomList arraylist to find the room that has the same ID. You
	 * can assume that there will always be a match.
	 * 
	 * @param id
	...
	/**
	 * Reads ands parses the information to set up the map by "connecting" rooms
	 * that are labeled to be adjacent to another.
	 *
	...
	/**
	 * Finds the room in the list that is the starting room. Create player object
	...
	/**
	 * Finds the room in the list that is the treasure room. Create dragon object
	 * and places them there.
	 */
	...
	/**
	 * Sets up the game by populating the rooms, player, dragon, and initial game
	 * state
	 * 
	 * @param roomInfoFile File The file that the room info should be read from
	 * @param adjFile      File The file that the adjacent room info should be read
	 *                     from
	 */
	 ...
	 * Then moves them. Handles teleportation as well.
	 * 
	...
	/**
	 * Continues the process of alternating between player and dragon turns until
	 * the game is over.
	 */
	private static void gamePlayLoop() {
		...
		while (currentGameState == GameState.CONTINUE) {
			...
			dragonTurn();
			// System.out.println("Dragon Location:" +dragon.getCurrentRoom()); //Can
			// comment back in to see where they are
