 ...
 * 
 * // TODO #1-9 below ask you to finish implementing the following methods (only
 * make edits below each of these numbered TODOs, as described for each):
 * peekTreasure() - returns largest treasure value from the root position of the
 * max-heap percolateDown() - recursively enforces heap ordering down from
 * specified index heapify() - iteratively moves values from a randomly-ordered
 * array into heap order removeBestTreasure() - removes treasure value from root
 * position in heap, and returns new size sortTreasure() - computes and returns
 * a sorted array of treasure values
 * 
 * Additionally, the following helper and test methods are provided: swap() -
 * swaps the values at two different positions within an array bestOf() -
 * returns the largest of its three string/null arguments testHeapify() -
 * returns true when this test passes and false otherwise testSort() - returns
 * true when this test passes and false otherwise main() - calls and prints out
...
public class TreasureSorter {
	...
	private static Treasure peekTreasure(Treasure[] heap) {
		// TODO #1: return element from the root position within the zero-indexed
		// max-heap
		return heap[0];
	...
	/**
	 * Recursive helper method which implements percolateDown operation to restore
	 * the heap order property if it is violated at a given index position of a
	 * provided max-heap.
	 * 
	 * @param index position of the element to start the percolate-down operation
	 *              from
	...
	private static void percolateDown(int index, Treasure[] heap, int size) {
		...
		// store values from index, left, and right child positions into separate
		// variables
		...
		if (best.equals(leftChildValueOrNull)) {
			// TODO #2: implement percolate down through left subtree - see provided utility
			// methods
			...
			// TODO #3: implement percolate down through right subtree - see provided
			// utility methods
			// below for some shortcuts!
	...
	/**
	 * Reorganizes an array of Treasure values into a valid max heap. This heapify
	 * operation does NOT operate in-place.
	 * 
	...
	/**
	 * Removes treasure value from root position in heap, and then return heap's new
	 * size. Assume that the provided array is NOT empty and is a valid max-heap.
	 * 
	 * @param heap a provided max-heap
	 ...
	 * This sort operation does NOT operate in-place. Assume that the provided array
	 * is NOT empty.
	 * 
	...
	public static Treasure[] sortTreasure(Treasure[] values, int size) {
		...
		while (heapSize > 0) {
			// TODO #8: insert best element from heap into earliest free position in sorted
			// array
	...
	/**
	 * Returns the largest between the provided arguments. Null arguments are
	 * ignored (considered smaller than Treasure).
	 * 
	...
	private static boolean testHeapify() {
		try {
			Treasure[] randomValues = new Treasure[] { new Treasure(3.1), new Treasure(64.0), new Treasure(1.6),
					new Treasure(98.6), new Treasure(32.0), new Treasure(9.8) };
			Treasure[] heapOfTreasure = new Treasure[] { new Treasure(98.6), new Treasure(64.0), new Treasure(9.8),
					new Treasure(3.1), new Treasure(32.0), new Treasure(1.6) };

			System.out.println("Expected: " + Arrays.deepToString(heapOfTreasure));
			System.out.println("Actual:   " + Arrays.deepToString(heapify(randomValues, randomValues.length)));
	...
	private static boolean testSort() {
		try {
			Treasure[] heapOfTreasure = new Treasure[] { new Treasure(98.6), new Treasure(32.0), new Treasure(64.0),
					new Treasure(1.6), new Treasure(3.1), new Treasure(9.8) };
			Treasure[] sortedTreasure = new Treasure[] { new Treasure(98.6), new Treasure(64.0), new Treasure(32.0),
					new Treasure(9.8), new Treasure(3.1), new Treasure(1.6) };

			System.out.println("Expected: " + Arrays.deepToString(sortedTreasure));
			System.out.println("Actual:   " + Arrays.deepToString(sortTreasure(heapOfTreasure, heapOfTreasure.length)));
