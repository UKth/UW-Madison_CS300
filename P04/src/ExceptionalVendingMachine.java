...
public class ExceptionalVendingMachine {
	...
	public void addItem(String description, int expirationDate) {
		...
		if (description == "" || description == null || expirationDate < 0) {
	...
	public String getItemAtIndex(int index) {
		// TODO complete the implementation of this method with respect to its above
		// specification
	...
	public int getIndexNextItem(String description) {
		...
		if (index == -1) {
			throw new NoSuchElementException("Can't find an matched item");
		}
	...
	public void loadOneItem(String itemRepresentation) throws DataFormatException {
		if ( itemRepresentation == null || itemRepresentation.trim().equals("")) {
			throw new IllegalArgumentException("Invalid itemRepresentation");
		}

		String[] itemInfos = itemRepresentation.split(":");
		if (itemInfos.length != 2 || itemInfos[0].trim().equals("") || itemInfos[1].trim().equals("")) {
			throw new DataFormatException("Invalid itemRepresentation format");
		}
		int n = -1;

		try {
			n = Integer.parseInt(itemInfos[1].trim());
	 ...
	 */
	public int loadItems(File file) throws FileNotFoundException, DataFormatException {
		...
		} catch (IllegalStateException e) { // handle checked exception
