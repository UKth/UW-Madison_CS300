...
/**
 * This class implements methods to store and manage the list of courses in a
 * campus. The list of courses is stored in an oversize array defined by a
 * reference to an array of strings which stores course labels and an int
 * variable which keeps track of the size of the array. Examples of course
 * labels: "cs200", "cs300", "me407", "ece252", etc.
 * 
 * We assume that all the course labels are correctly formatted.
 * 
 * The array courses is an oversize array. This means it contains non null
 * references from index 0 to index size-1. All references in the range of
 * indexes size .. courses.length-1 MSUT be null.
 * 
 * In this courseList class, new courses are added to the end of the array.
 * Removal operations are made from the beginning of the array.
 * 
...
public class CourseList {
	...
	/**
	 * Inserts a new course to the end of the oversize array of courses defined by
	 * courses and size.
	 * 
	 ...
	 * @return the size of the oversize array after trying to insert newCourse to
	 *         the list of courses
	 * @throws IllegalStateException if the array courses is full
	 */
	public static int addLast(String[] courses, int size, String newCourse) throws IllegalStateException {
		// TODO
		...
		if (courses.length == size) {
			throw new IllegalStateException("courses is full");
	...
	/**
	 * Gets the number of computer science courses in the course list defined by
	 * courses and size. A CS course is identified by the two first characters of
	 * its label, which must be equal to "cs".
	 * 
	 * @param courses an array which stores strings representing labels of courses
	 *                taught in a campus
	...
	/**
	 * Removes the course stored at index zero of the oversize array defined by
	 * courses and size.
	 * 
	 ...
	 * @return the size of the oversize array after trying to remove the course at
	 *         index 0.
	 */
	...
	/**
	 * Checks whether the correctness of the CourseList.addLast() method when passed
	 * a non-full oversize array.
	 * 
	...
	public static boolean testSuccessfulAddLast() {
		// TODO
		// 1. Create an NON-full oversize array of Strings which already contains 3
		// courses.
		
		String[] courses = new String[] { "cs200", "ece252", "cs300", null, null, null };
		int size = 3;
		int expectedCount = 4; // expected output
		String[] expected = new String[] { "cs200", "ece252", "cs300", "art101", null, null };
		try {
			
			if (CourseList.addLast(courses, size, "art101") != expectedCount)
				return false;
	 ...
	 * CourseList.getCsCoursesCount() method when passed an non-empty array
	 * containing a few cs courses.
	 * 
	 * @return true if the method passes this test and false otherwise
	 */
	public static boolean testGetCsCoursesCount() {
		// This tester method does not check for all test scenarios to check the
		// correctness of
		// getCsCoursesCount() method. This is NOT a programming assignment. You do not
		// need to check
		// for
		// everything in an exam.

		// test scenario: non-empty oversize array of course labels
		String[] courses = new String[] { "cs200", "ece252", "cs300", "art101", "me407", "cs502", null, null, null };
		int size = 6;
		int expectedCount = 3; // expected output
		String[] original = new String[] { "cs200", "ece252", "cs300", "art101", "me407", "cs502", null, null, null };
		try {
			// call getCsCoursesCount() and check whether it returns the expected output
			// without making
			// any
	...
	/**
	 * Checks whether the CourseList.removeFirst method works as expected when
	 * called to remove the course at index zero from a non-empty oversize array of
	 * Strings
	 *
	 * @return true when this test verifies a correct functionality, and false
	 *         otherwise
	...
	public static boolean testFirstCourseRemoval() {
		...
		String[] courses = new String[] { "cs240", "art200", "cs300", "ece252", "cs400", null, null, null };
		int size = 5;
		// expected output and array after removing the first course
		String[] expectedCourses = new String[] { "art200", "cs300", "ece252", "cs400", null, null, null, null };
